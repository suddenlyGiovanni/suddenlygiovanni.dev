...
export function Interests({
	...
	return (
		<section class="w-full">
			<T.h2 class="mb-0">Interests</T.h2>
			{interests.map(({ name, keywords }) => (
				<dl key={name}>
					<dt>{name}</dt>
					<dd>
						<ul class="mb-0 ml-0 flex list-none flex-row flex-wrap items-start justify-start">
							{keywords?.map(keyword => (
								<li class="mr-4 mb-0" key={generateDjb2Hash(keyword)}>
									{keyword}
								</li>
							))}
						</ul>
